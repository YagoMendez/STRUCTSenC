...
int ingreso(registro alumno[cantAlum], int flag){
...
void mostrarAlumnos(registro alumno[], int flag){
    ...
    if(flag >= 2){
        for (int i = 0; i < flag; i++){//Orden de los alumnos segun promedio
    ...
    for(int k = 0; k <= flag; k++){
        p("\n%3d      %3s      %3d\n", alumno[k].cod, alumno[k].apyno, alumno[k].promedio);
    }
}   

void concatenar(registro *alumno, char nom[50], char ape[50], int i){
    int tamNom = strlen(nom);

    strcpy(alumno[i].apyno,nom);
    alumno[i].apyno[tamNom] = ' ';
    strcpy(alumno[i].apyno + tamNom + 1, ape);
}
